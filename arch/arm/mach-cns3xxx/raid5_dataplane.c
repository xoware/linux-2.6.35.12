...
int check_xor_status()
{
	...
	if((*(dma->cregs->stat)	& (REG_STAT_XFER_COMPLETE | 
		...
		//printk("%s: pstat=0x%08x\n", __FUNCTION__, *(dma->cregs->stat));
                        ...
                        void *dst_ptr, sg_t *sg)
{
	...
	if(unlikely(!dst_ptr)) {
		...
		if(likely(bh_ptr[i])) {
                	sg->entry[j] = (SG_ADDR_MASK & ((u64)virt_to_phys(bh_ptr[i])))
                           		| (SG_READ_IDX_MASK & ((u64)j + 1) << SG_IDX_SHIFT)
                                           | (RWI_RD_D);
			j++;
		}
        }
