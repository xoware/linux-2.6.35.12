...
static struct class *rdma_class = NULL;
...
extern void rdma_unit_test(int, unsigned int, unsigned int);

static ssize_t rdma_debug_show(struct class *c, struct class_attribute *attr, char *b)
{
	int len = 0, length = 0;
	char *p = b;
	...
	len = sprintf(p, "reg->ParaReg(0x%p, 0x%x) = 0x%08x / 0x%08x\n", 
					(void *)dma->cregs->para, 
					RDMA_REGS_PHYS(REG_PARA_OFFSET), RDMA_REGS_VALUE(REG_PARA_OFFSET), 
					*(dma->cregs->para));
    LPLUS;
	len = sprintf(p, "reg->BlkSize(0x%p, 0x%x) = 0x%08x / 0x%08x\n", 
					(void *)dma->cregs->blsz, 
					RDMA_REGS_PHYS(REG_BLSZ_OFFSET), RDMA_REGS_VALUE(REG_BLSZ_OFFSET), 
					*(dma->cregs->blsz));
    LPLUS;
	len = sprintf(p, "reg->S/GAddr(0x%p, 0x%x) = 0x%08x / 0x%08x\n", 
					(void *)dma->cregs->sgad, 
					RDMA_REGS_PHYS(REG_SGAD_OFFSET), RDMA_REGS_VALUE(REG_SGAD_OFFSET), 
					*(dma->cregs->sgad));
    LPLUS;
	len = sprintf(p, "reg->StatReg(0x%p, 0x%x) = 0x%08x / 0x%08x\n", 
					(void *)dma->cregs->stat,
					RDMA_REGS_PHYS(REG_STAT_OFFSET), RDMA_REGS_VALUE(REG_STAT_OFFSET), 
					*(dma->cregs->stat));
    LPLUS;
	len = sprintf(p, "reg->FrntPtr(0x%p, 0x%x) = 0x%08x / 0x%08x\n", 
					(void *)dma->cregs->frnt, 
					RDMA_REGS_PHYS(REG_FRNT_OFFSET), RDMA_REGS_VALUE(REG_FRNT_OFFSET), 
					*(dma->cregs->frnt));
    LPLUS;
	len = sprintf(p, "reg->BackPtr(0x%p, 0x%x) = 0x%08x / 0x%08x\n", 
					(void *)dma->cregs->back, 
					RDMA_REGS_PHYS(REG_BACK_OFFSET), RDMA_REGS_VALUE(REG_BACK_OFFSET), 
					*(dma->cregs->back));
    LPLUS;
	len = sprintf(p, "reg->QuParam(0x%p, 0x%x) = 0x%08x / 0x%08x\n", 
					(void *)dma->cregs->qpar, 
					RDMA_REGS_PHYS(REG_QPAR_OFFSET), RDMA_REGS_VALUE(REG_QPAR_OFFSET), 
					*(dma->cregs->qpar));
    LPLUS;
...
static ssize_t rdma_debug_store(struct class *c, struct class_attribute *attr, const char *b, size_t count)
{
	...
	if (*b != '0' || (b[1] != 'x' && b[1] != 'X'))
		...
		sscanf(b, "0x%x 0x%x 0x%x", &unit_test_action, &src_cnts, &bytes);
	else
		sscanf(b, "0X%x 0X%x 0X%x", &unit_test_action, &src_cnts, &bytes);
...
static ssize_t timeout_ms_show(struct class *c, struct class_attribute *attr, char *b)
{
	int len = 0, length = 0;
	char *p = b;
...
static ssize_t timeout_ms_store(struct class *c, struct class_attribute *attr, const char *b, size_t count)
{
	unsigned int t_ms;
