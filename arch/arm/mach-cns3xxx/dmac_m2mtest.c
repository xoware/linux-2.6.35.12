...
MODULE_PARM_DESC(lm2m_dt,"Number of data transfers for large m2m test");
                                             // MIN is 128 bytes
#define LM2M_BUFFER_SIZE    256*1024      // MAX bytes to transfer for lm2m
...
{
    int i;
    for (i=0;((i < sz) && ((*(p+i)) == c));i++);
    if (i == sz) return 1; // success
    printk ("Broken at byte %d\n",i);
    return 0;
}
...
static int do_large_m2m_test (void)
{
    ...
    {
        printk ("Invalid BPB Param .. setting defaults and proceeding\n");
        lm2m_bpb = 8;
    }
    if ((lm2m_dt < 1) || (lm2m_dt > 16))
    {
        printk ("Invalid DT Param .. setting defaults and proceeding\n");
        lm2m_dt = 2;
    }
    bpb_val = get_bpb_val (lm2m_bpb);
    
    lm2m_src = dma_alloc_coherent (NULL, LM2M_BUFFER_SIZE,&lm2m_src_paddr,GFP_KERNEL);
    lm2m_dst = dma_alloc_coherent (NULL, LM2M_BUFFER_SIZE,&lm2m_dst_paddr,GFP_KERNEL);
    ...
    {
        printk ("Got the physical addresses - lm2m_src: %x, lm2m_dst: %x\n",lm2m_src_paddr, lm2m_dst_paddr);
    }
    ...
    printk ("Filling the lm2m_src address %p [%d] with character 'c' for size %d ..",
            lm2m_src, lm2m_src_paddr, LM2M_BUFFER_SIZE);
    memset (lm2m_src, 'c', LM2M_BUFFER_SIZE);
    if (mem_verify((uint8_t*)lm2m_src, 'c', LM2M_BUFFER_SIZE )) 
        printk (" Verified Memory fill\n");

    printk ("Filling the lm2m_dst address %p [%d] with character 'a' for size %d ..",
            lm2m_dst, lm2m_dst_paddr, LM2M_BUFFER_SIZE);
    ...
    if (lm2m_lc1  > 0x100)
    {
        printk ("%s:%d > Warning : uncertain bpb setting ... test may fail\n"
                ,__FUNCTION__,__LINE__);
        lm2m_lc1 &= 0xff;
    }

    lm2m_ch = dmac_get_channel (NULL,NULL);
    printk ("Got the Free channel : %d. Proceeding with ctrl: %x\n",lm2m_ch,ctrl_value);
    if ((lm2m_ch <0) || (lm2m_ch>7))
    {
        printk ("Got an invalid channel no\n");
        return -1;
    }

    {
	    ...
	    DMAC_DMALP (lm2m_ch, 0, lm2m_lc0);
	        DMAC_DMALD (lm2m_ch);
	        DMAC_DMAST (lm2m_ch);
	    DMAC_DMALPEND (lm2m_ch,0, DMAST_INSTR_SIZE + DMALD_INSTR_SIZE, 1);
	    DMAC_DMALPEND (lm2m_ch,1, DMALPEND_INSTR_SIZE + DMAST_INSTR_SIZE + DMALD_INSTR_SIZE + DMALP_INSTR_SIZE, 1);
...
static int do_simple_m2m_test (void)
{
    ...
    if ((bytes_to_transfer < 4) || (bytes_to_transfer > SIMP_MAX_BYTES_TO_TRANSFER))
    {
        printk ("Invalid bytestotransfer param. Limit b/w 4 and 2048\n");
        bytes_to_transfer = SIMP_MAX_BYTES_TO_TRANSFER;
    }

    buf_src = dma_alloc_coherent (NULL,BUFFER_SIZE*4,&buf_src_paddr,GFP_KERNEL|GFP_DMA);
    buf_dst = dma_alloc_coherent (NULL,BUFFER_SIZE*4,&buf_dst_paddr,GFP_KERNEL|GFP_DMA);
    ...
    {
        printk ("Got the physical addresses - src: %x, dst: %x\n",buf_src_paddr, buf_dst_paddr);
    }
    ...
    printk ("Filling the src address %p [%d] with character 'c' for size %d ..",
            buf_src, buf_src_paddr, BUFFER_SIZE*4);
    memset (buf_src, 'c', BUFFER_SIZE*4);
    if (mem_verify((uint8_t*)buf_src, 'c', BUFFER_SIZE *4)) 
        printk (" Verified Memory fill\n");
    
    printk ("Filling the dst address %p [%d] with character 'a' for size %d ..",
            buf_dst, buf_dst_paddr, BUFFER_SIZE*4);
    ...
    {
        printk ("bpb(%d)*dt(%d) is greater than bytes_to_transfer(%d) .. fine tune your options\n",bpb,dt,bytes_to_transfer);
    ...
    if (lc0>0x100)
    {
        printk ("%s:%d > Invalid bpb value [%d].. transfer size is %d, use lm2m instead\n",__FUNCTION__,__LINE__,bpb,bytes_to_transfer);
    }
...
{
    printk ("OK, interrupt generated with ev no %d\n",itest_ch);
    return 0;
}
...
static int do_itest (void)
{
    ...
    {
        printk ("Debug this !! got invalid channel [%d]\n",itest_ch);
        return -1;
    }

    printk ("Testing interrupt with channel no %d\n",itest_ch);
...
int es_m2m_inthandler (void *chval)
{
    if (!chval) 
    {
        printk ("BUG Here [%s:%d] .. debug\n",__FUNCTION__,__LINE__);
        return -1;
    }
    printk ("Channel %d generated an interrupt\n",*((int*)chval));
    return 0;
}
...
static int do_eventsync_2ch_m2m_test (void)
{
    ...
    es_buf_src = dma_alloc_coherent (NULL,EVENTSYNC_2CH_BUFFER_SIZE,&es_buf_src_paddr,GFP_KERNEL|GFP_DMA);
    es_buf_via = dma_alloc_coherent (NULL,EVENTSYNC_2CH_BUFFER_SIZE,&es_buf_via_paddr,GFP_KERNEL|GFP_DMA);
    es_buf_dst = dma_alloc_coherent (NULL,EVENTSYNC_2CH_BUFFER_SIZE,&es_buf_dst_paddr,GFP_KERNEL|GFP_DMA);

    if ( !es_buf_src || !es_buf_via || !es_buf_dst)
    {
        if (es_buf_src) 
            dma_free_coherent (NULL,EVENTSYNC_2CH_BUFFER_SIZE,es_buf_src, es_buf_src_paddr);
        if (es_buf_via) 
            dma_free_coherent (NULL,EVENTSYNC_2CH_BUFFER_SIZE,es_buf_via, es_buf_via_paddr);
        if (es_buf_dst) 
            dma_free_coherent (NULL,EVENTSYNC_2CH_BUFFER_SIZE,es_buf_dst, es_buf_dst_paddr);

        printk ("%s:%d > Memory allocation failed.fix this first\n",__FUNCTION__
                ,__LINE__);
        return -1;
    }
    ...
    if (mem_verify((uint8_t*)es_buf_src, 'c', EVENTSYNC_2CH_BUFFER_SIZE))
        printk ("Filled Src Buffer (va:%p, pa: %x) with char 'c'\n",es_buf_src,
                es_buf_src_paddr);

    if (mem_verify((uint8_t*)es_buf_dst, 'a', EVENTSYNC_2CH_BUFFER_SIZE))
        printk ("Filled Dst Buffer (va:%p, pa:%x) with char 'a'\n",es_buf_dst,es_buf_dst_paddr);
    ...
    if (!VALID_CHANNEL(es_ch1))
    {
        ...
        dma_free_coherent (NULL,EVENTSYNC_2CH_BUFFER_SIZE,es_buf_via, es_buf_via_paddr);
        dma_free_coherent (NULL,EVENTSYNC_2CH_BUFFER_SIZE,es_buf_dst, es_buf_dst_paddr);
        printk ("%s:%d: Got Invalid channel. Debug this\n",__FUNCTION__,__LINE__);
        return -1;
    }
    printk ("Got Channel es_ch1 = %d\n",es_ch1);

    if (dmac_get_event(es_ch1, es_ch1_evt) == -1)
    {
        printk ("%s:%d > get_event failed\n",__FUNCTION__,__LINE__);
        dma_free_coherent (NULL,EVENTSYNC_2CH_BUFFER_SIZE,es_buf_src, es_buf_src_paddr);
        dma_free_coherent (NULL,EVENTSYNC_2CH_BUFFER_SIZE,es_buf_via, es_buf_via_paddr);
        dma_free_coherent (NULL,EVENTSYNC_2CH_BUFFER_SIZE,es_buf_dst, es_buf_dst_paddr);
        dmac_release_channel (es_ch1);
    }
    printk ("Got event no %d for Channel %d\n",es_ch1_evt,es_ch1);
    ...
    ctrl_value = 
                    ...
                    (0x2 << 22)|  // dst_prot_ctrl
                    (0x0 << 25);  // dst_cache_ctrl
    printk ("Got the bpb_val as %x and ctrl is %x\n",es_bpb_val,ctrl_value);

    {
        ...
        DMAC_DMALPEND (es_ch1, 1, DMALPEND_INSTR_SIZE + DMAWFE_INSTR_SIZE + 
                        + DMASEV_INSTR_SIZE 
                        + DMAST_INSTR_SIZE + DMALD_INSTR_SIZE 
                        + DMAWMB_INSTR_SIZE + DMALP_INSTR_SIZE, 1);
        DMAC_DMAEND (es_ch1);
    }
...
static void do_verify_es_m2m_test (void)
{
    ...
    {
        printk ("\n**ESM2M Result : Verified bytes [%d].. OK\n",es_bytes_to_transfer);
    }
    else
    {
        int i=0;
        printk ("Dumping dst buf\n");
        for (i=0;i <13; i++)
            printk ("%x ",*((uint8_t*)es_buf_dst+i));
        printk ("\n**ESM2M Result : Verification of bytes [%d].. FAILED [%c %c]\n",es_bytes_to_transfer,*((uint8_t*)es_buf_dst),*((uint8_t*)es_buf_dst+1));
    }
        dma_free_coherent (NULL,EVENTSYNC_2CH_BUFFER_SIZE,es_buf_src, es_buf_src_paddr);
        dma_free_coherent (NULL,EVENTSYNC_2CH_BUFFER_SIZE,es_buf_via, es_buf_via_paddr);
        dma_free_coherent (NULL,EVENTSYNC_2CH_BUFFER_SIZE,es_buf_dst, es_buf_dst_paddr);
...
static void do_verify_lm2m_test (void)
{
    ...
    {
        printk ("\n**LM2M Result : Verification of bytes [%d].. FAILED\n",lm2m_bytes_to_transfer);
    }
...
static void do_verify_simple_m2m_test (void)
{
    ...
    {
        printk ("\n**Result : Verification of bytes [%d].. FAILED\n",bytes_to_transfer);
    }

    if (*((uint8_t*)buf_dst+bytes_to_transfer) == 'c')
    {
        printk ("One byte after the limit is also written .. check this\n");
    }
...
static int __init dmac_dmatest_init (void)
{
    ...
    {
        printk ("usage : insmod dma_m2m_test.ko <options>\n");
        printk ("options:\n");
        printk ("s=1\t\t To enable Simple M2M Test\n");
        printk ("l=1\t\t To enable Large M2M Test\n");
        printk ("es=1\t\t To enable 2 chan synch M2M Test\n");
        return -1;
    }
    ...
    if (s == 1)
    {
        ...
        {
            printk ("Mem Test Setup Failure for simple m2m ... bailing out\n");
            return -1;
        }
        dmac_dump_regs();
    }
    ...
    if (l == 1)
    {
        ...
        {
            printk ("Mem Test Setup Failure for large m2m ... bailing out\n");
            if (s == 1) do_verify_simple_m2m_test ();
            return -1;
        }
        dmac_dump_regs();
    }
    ...
    if (es == 1)
    {
        ...
        {
            printk ("Setup Failure for eventsync 2ch m2m test .. bailing\n");
            if (s == 1) do_verify_simple_m2m_test ();
            if (l == 1) do_verify_lm2m_test ();
            return -1;
        }
        dmac_dump_regs();
    }
