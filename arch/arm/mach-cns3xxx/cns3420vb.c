...
static unsigned long cns3xxx_camera_query_bus_param(struct soc_camera_link *link)
{
        int ret;

        if (!gpio_bus_switch) {
                ret = gpio_request(128 + 1, "camera");
                if (!ret) {
                        gpio_bus_switch = 128 + 1;
...
static void __init cns3420_init(void)
{
	...
	l2x0_init((void __iomem *) CNS3XXX_L2C_BASE_VIRT, 0x00540000, 0xfe000fff);
#endif
	...
	printk("Cavium feature enabled: "
		...
		"64KB page size, "
#endif
#ifdef CONFIG_CNS3XXX_JUMBO_FRAME_64K
		"Jumbo frame, "
#endif
	"\n");
