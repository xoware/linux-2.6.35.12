...
#if 0
	printk("%s: clk_save 0x%.8x pll_save_0x%.8x\n", 
				__FUNCTION__, clk_save, pll_save);
	printk("%s: clk_save & ~(CNS_PWR_CLK_MASK) 0x%.8x pll_save | (CNS_PWR_PLL_MASK) 0x%.8x\n", 
				__FUNCTION__, clk_save & ~(CNS_PWR_CLK_MASK), pll_save | (CNS_PWR_PLL_MASK));
#endif
#endif
...
void cns3xxx_secondary_suspend()
{
...
void cns_pwr_doze(void)
{
...
#else
        wakeup_set(IRQ_CNS3XXX_EXTERNAL_PIN2);
#endif /* CNS3XXX_PSE_WOL */
	...
	printk("<0> CP-core moving to WFI\n");
#endif /* CONFIG_AMP */
