...
struct proc_dir_entry *amp_proc_dir=NULL;
...
extern void dp_csd_unlock(void);
...
{
	struct task_struct *idle = NULL; /*TODO: This needs to be removed later*/
        pgd_t *pgd;
        pmd_t *pmd;
        int ret;
	...
	secondary_data.stack = (void *) kmalloc(DATA_PLANE_STACK_SIZE, GFP_KERNEL);
	
	if (secondary_data.stack == NULL)
	{
		printk(KERN_EMERG "CPU%d: Stack Allocation failed for Secondary Core\n", cpunum);
		return -ENOMEM;
	}
	...
	amp_proc_dir = proc_mkdir("amp", NULL);
        ...
        ret = boot_secondary(cpu, idle); /*TODO: idle is not required, to be removed later*/
        if (ret == 0) {
...
void (*dp_entry_func)(void) = NULL;
unsigned int cpu2_proc = 0;
...
#else
dp_wfi_state:
        printk("<0> DP-core moving to WFI\n");
#ifdef CONFIG_PM
        ...
        printk("CPU%u DP-core coming out of WFI\n", cpu);
