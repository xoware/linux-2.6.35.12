...
//KH
// ----------------------------------------------
// 20110628: move define to libata for module
struct proc_dir_entry *sata_proc_entry;
extern long unsigned int last_jiffs;
extern unsigned int sata_stagger_enable;
extern unsigned int sata_disk_standby[2][STAGGER_SATA_PORT_DISKS];
/*  ------------------------------------  */
      ...
      for(i=0;i<2;i++)
         {
            ...
            for(j=0;j<STAGGER_SATA_PORT_DISKS;j++)
               {
                  num += sprintf(page + num , "  channel[%d] = %d\n",j,sata_disk_standby[i][j]);
               }
            num += sprintf(page + num ,"\n");
         }
      num += sprintf(page + num , "\n\nlast_jiffs          = %lx\n",last_jiffs);
      num += sprintf(page + num , "jiffies             = %ld\n",jiffies);
      num += sprintf(page + num , "\n\nsata_stagger_enable = %d\n\n",sata_stagger_enable);

      return num;
   }
static int sata_write_proc(struct file *file, const char *buffer,unsigned long count, void *data)
   {
      if(count)
         {
            unsigned int i = 0;
            ...
            if(buffer[0]=='s'&&buffer[1]=='t'&&buffer[2]=='a'&&buffer[3]=='g'&&buffer[4]=='g'&&buffer[5]=='e'&&buffer[6]=='r')
               {
                  sata_stagger_enable = simple_strtol(buffer+8, 0, 0);
                  if(sata_stagger_enable>=1){sata_stagger_enable=1;}
                  else{sata_stagger_enable=0;}
                  
                  /*
                  for(i=0;i<5;i++)
                     {
                        if(sata_disk_standby[0][i]==2){spindown++;}
                        if(sata_disk_standby[1][i]==2){spindown++;}
                     }
	...
	if (sata_proc_entry)
	   {
	      int i=0;
              ...
              for(i=0;i<STAGGER_SATA_PORT_DISKS;i++)
                 {
                    sata_disk_standby[0][i] = 0;
                    sata_disk_standby[1][i] = 0;
                 }
           }
